...
class ExperienceBuffer:
    ...
    def _init_from_env_info(self, env_info):
        ...
        self.tensor_dict['future_states'] = torch.zeros(
        self.horizon_length, self.num_agents * self.num_actors, sequence_ahead_steps, 45,
        dtype=torch.float32, 
        device=self.device
        )
        
        # For future actions - using hardcoded dimension of 12 for actions
        self.tensor_dict['future_actions'] = torch.zeros(
            self.horizon_length, self.num_agents * self.num_actors, sequence_ahead_steps, 12,
            dtype=torch.float32, 
            device=self.device
        )
        
        # For future dones
        self.tensor_dict['future_dones'] = torch.zeros(
            self.horizon_length, self.num_agents * self.num_actors, sequence_ahead_steps,
            dtype=torch.float32, 
            device=self.device
        )

        # For future_reward
        self.tensor_dict['future_reward'] = torch.zeros(
            self.horizon_length, self.num_agents * self.num_actors, sequence_ahead_steps,
            dtype=torch.float32, 
            device=self.device
        )
