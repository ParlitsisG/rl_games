...
class BaseModelNetwork(nn.Module):
    def __init__(self, obs_shape, normalize_value, normalize_input, value_size):
        ...
        self.curiosity_mean_std = RunningMeanStd((self.value_size,),norm_only=True)
        if normalize_value:
...
class ModelA2CContinuousLogStd(BaseModel):
    ...
    class Network(BaseModelNetwork):
        ...
        def forward(self, input_dict):
            ...
            prev_actions = input_dict.get('prev_actions', None)
            input_dict['obs'] = self.norm_obs(input_dict['obs'])
            mu, logstd, value, states, curiosity_loss= self.a2c_network(input_dict)
            sigma = torch.clamp(torch.exp(logstd),0.0000001)
            distr = torch.distributions.Normal(mu, sigma, validate_args=False)
            if is_train:
                entropy = distr.entropy().sum(dim=-1)
