...
class RunningMeanStdObs(nn.Module):
    def __init__(self, insize, epsilon=1e-05, per_channel=False, norm_only=False):
        ...
        self.skip_keys=["length",'actions']
        super(RunningMeanStdObs, self).__init__()
        self.running_mean_std = nn.ModuleDict({
            k : RunningMeanStd(v, epsilon, per_channel, norm_only) for k,v in insize.items() if k not in self.skip_keys
        })
    ...
    def forward(self, input, denorm=False):
        ...
        for k,v in input.items():
            if k in self.skip_keys:
                res[k] = v  # Pass through unchanged
            else:
